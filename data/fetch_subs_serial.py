#!/usr/bin/env python3
"""yt-dlp channel_follower_count - 직렬 shell 실행 (안정적)"""
...
for idx, d in targets:
    url = d["channel_url"]
    # shell=True, quoting 없이 (URL에 공백 없음)
    cmd = f"yt-dlp --no-warnings --skip-download --playlist-end 1 --print %(channel_follower_count)s {url}"
    try:
        r = subprocess.run(cmd, capture_output=True, text=True, timeout=25, shell=True)
        out = r.stdout.strip()
        if out and out != "NA":
            try:
                subs = int(out)
